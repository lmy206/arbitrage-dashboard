...
4. 36 组指标及常规期货组合成交量最大的 4 个共同合约月写入 `app/data/arbitrage.json`，网站从该文件渲染。
5. Windows 计划任务在周一至周五 20:10 检查新交易日数据；校验和静态页面测试通过后，只提交 `app/data/arbitrage.json` 并推送 `main`。节假日因数据日不变而自动跳过。

## 图表

- 分位总览：全部 36 个品种对的近 5 年分位横向排名，统一使用 0%–100% 量尺。
...
`xtdata` 凭证只从 `XTQUANT_TOKEN` 环境变量或本机 `E:\data\config.py` 读取，不写入项目。若 xtdata 行情不可用、36 组数据不齐、批准的外部数据缺失或过期、交易日不一致或检测到未来数据，脚本保留上次有效网页数据并记录 `xtdata_unavailable`、`external_source_unavailable` 或校验错误，不发布异常结果。
