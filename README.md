...
4. 36 组主表指标、2 个展开利润观察口径及常规期货组合成交量最大的 4 个共同合约月写入 `app/data/arbitrage.json`，网站从该文件渲染。
...
- 价差与利润代理：主表保留塑料-聚丙烯价差=`L−PP`、MTO盘面利润=`PP−3×MA` 和 PTA盘面加工费=`TA−0.655×PX`；玻璃生产利润=`FG−0.2×SA` 放在“玻璃纯碱比”的展开观察口径中，焦化利润=`J−1.3×JM` 放在“焦炭焦煤比”的展开观察口径中。全部使用 xtdata `JQ00` 持仓量加权序列；展开利润观察配比分别按 `5:1` 与 `6:13` 展示，并各自提供分位判断与历史图。
...
`xtdata` 凭证只从 `XTQUANT_TOKEN` 环境变量或本机 `E:\data\config.py` 读取，不写入项目。若 xtdata 行情不可用、36 组主表数据或2个展开利润观察不齐、批准的外部数据缺失或过期、交易日不一致或检测到未来数据，脚本保留上次有效网页数据并记录 `xtdata_unavailable`、`external_source_unavailable` 或校验错误，不发布异常结果。
