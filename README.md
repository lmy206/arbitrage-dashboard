...
- IM-IC价差：中证1000股指期货减中证500股指期货；卷螺价差：热卷减螺纹钢；螺矿比：螺纹钢除以铁矿石；焦炭/焦煤比：焦炭除以焦煤；豆粕价差：豆粕减豆一；金银比：黄金价格乘 1000 后除以白银价格。
- 新增比价：玻璃/纯碱、聚丙烯/甲醇、燃料油/沥青、20号胶/BR橡胶、烧碱/玻璃、镍/不锈钢、猪肉/玉米均按左腿除以右腿计算。玻璃与聚乙烯、聚丙烯缺少直接产业锚，已从看板删除。
- 价差与利润代理：主表保留塑料-聚丙烯价差=`L−PP`、MTO盘面利润=`PP−3×MA` 和 PTA盘面加工费=`TA−0.655×PX`；玻璃生产利润=`FG−0.2×SA` 放在“玻璃/纯碱比”的展开观察口径中并排在加权行上方，焦化利润=`J−1.3×JM` 放在“焦炭/焦煤比”的展开观察口径中并排在加权行上方。全部使用 xtdata `JQ00` 持仓量加权序列；展开利润观察配比分别按 `5:1` 与 `6:13` 展示，并各自提供分位判断与历史图。
