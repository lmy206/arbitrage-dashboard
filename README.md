...
4. 38 组主表指标、3 个展开利润观察口径及常规期货组合成交量最大的 4 个共同合约月写入 `app/data/arbitrage.json`，网站从该文件渲染。
...
- 分位总览：全部 38 个主表品种对的近 5 年分位横向排名，统一使用 0%–100% 量尺。
...
- 商品组合：蛋白质价差（豆粕减菜粕）、豆棕价差等均使用 xtdata `JQ00` 加权序列作为默认展示口径；科创50/上证50、创业板/沪深300使用 xtdata 现货指数，只作风格参考，因此不展示平衡手数、名义值和保证金。
- IM-IC价差：中证1000股指期货减中证500股指期货；卷-螺价差：热卷减螺纹钢；螺/矿比价：螺纹钢除以铁矿石；焦炭/焦煤比：焦炭除以焦煤；豆粕价差：豆粕减豆一；油/粕比价：豆油除以豆粕；金/银比价：黄金价格乘 1000 后除以白银价格。
- 新增比价：玻璃/纯碱、铜/铝、聚丙烯/甲醇、燃料油/沥青、20号胶/BR橡胶、烧碱/玻璃、镍/不锈钢、猪肉/玉米均按左腿除以右腿计算。玻璃与聚乙烯、聚丙烯缺少直接产业锚，已从看板删除。
- 价差与利润代理：主表保留塑料-聚丙烯价差=`L−PP` 和 PTA盘面加工费=`TA−0.655×PX`；MTO盘面利润=`PP−3×MA` 放在“聚丙烯/甲醇比价”的展开观察口径中，玻璃生产利润=`FG−0.2×SA` 放在“玻璃/纯碱比价”的展开观察口径中，焦化利润=`J−1.3×JM` 放在“焦炭/焦煤比”的展开观察口径中，三项均排在各自加权行上方。全部使用 xtdata `JQ00` 持仓量加权序列；展开利润观察配比分别按 `2:3`、`5:1` 与 `6:13` 展示，并各自提供分位判断与历史图。
- 涤纶短纤生产利润价差：`PF−0.86×PTA−0.34×MEG`，使用 xtdata `JQ00` 持仓量加权序列，单位为元/吨，未扣能源、人工等其他加工成本。合约月份按共同月份匹配，参考手数 `6:5:1`，即30吨短纤对应25吨PTA与10吨MEG，最大原料单耗偏差约3.1%。
...
`xtdata` 凭证只从 `XTQUANT_TOKEN` 环境变量或本机 `E:\data\config.py` 读取，不写入项目。若 xtdata 行情不可用、38 组主表数据或3个展开利润观察不齐、批准的外部数据缺失或过期、交易日不一致或检测到未来数据，脚本保留上次有效网页数据并记录 `xtdata_unavailable`、`external_source_unavailable` 或校验错误，不发布异常结果。
