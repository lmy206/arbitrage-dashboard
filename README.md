...
- 商品组合：蛋白质价差（豆粕减菜粕）、豆棕价差、棕榈油/豆油比价等均使用 xtdata `JQ00` 加权序列作为默认展示口径；棕榈油/豆油比价按 `P/Y` 计算并将月份展开限制为1、5、9月共同合约。科创50/上证50、创业板/沪深300使用 xtdata 现货指数，只作风格参考，因此不展示平衡手数、名义值和保证金。
