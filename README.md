...
- 中证1000叠加：仅在 IM/IF 比价与 IM-IC 价差展开区的现货指数图右轴叠加中证1000现货点位，便于对照相对价格与小盘指数方向；主连图、月份图和其他套利对不叠加。两张图右上角同时展示完整日频的10年水平 Pearson 相关系数和日变动 Pearson 相关系数；比价使用日收益率，价差使用一阶差分，中证1000统一使用日收益率。
