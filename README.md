...
4. 38 组指标及常规期货组合成交量最大的 4 个共同合约月写入 `app/data/arbitrage.json`，网站从该文件渲染。
5. Windows 计划任务在周一至周五 20:10 检查新交易日数据；校验和静态页面测试通过后，只提交 `app/data/arbitrage.json` 并推送 `main`。节假日因数据日不变而自动跳过。

## 图表

- 分位总览：全部 38 个品种对的近 5 年分位横向排名，统一使用 0%–100% 量尺。
...
- 新增比价：玻璃/纯碱、燃料油/沥青、20号胶/BR橡胶、烧碱/玻璃、镍/不锈钢、猪肉/玉米均按左腿除以右腿计算。玻璃与聚乙烯、聚丙烯缺少直接产业锚，已从看板删除。
- 价差与利润代理：塑料-聚丙烯价差=`L−PP`；MTO盘面利润=`PP−3×MA`，是未扣固定加工费、副产品、运输等项目的简化屏幕利润；PTA盘面加工费=`TA−0.655×PX`，未扣其他加工及物流成本；玻璃生产利润=`FG−0.2×SA`，未扣燃料与其他成本；焦化利润=`J−1.3×JM`，未扣加工与其他成本。五项均使用 xtdata `JQ00` 持仓量加权序列，并在月份展开区按同到期月计算；平衡手数分别按 `1:1`、`2:3`、单边不超过 20 手时最接近 0.655 单耗的 `20:13`、玻璃利润公式系数对应的 `5:1`，以及焦化利润公式与合约乘数对应的 `6:13` 展示。
...
`xtdata` 凭证只从 `XTQUANT_TOKEN` 环境变量或本机 `E:\data\config.py` 读取，不写入项目。若 xtdata 行情不可用、38 组数据不齐、批准的外部数据缺失或过期、交易日不一致或检测到未来数据，脚本保留上次有效网页数据并记录 `xtdata_unavailable`、`external_source_unavailable` 或校验错误，不发布异常结果。
