...
- 化工价差与利润代理：塑料-聚丙烯价差=`L−PP`；MTO盘面利润=`PP−3×MA`，是未扣固定加工费、副产品、运输等项目的简化屏幕利润；PTA盘面加工费=`TA−0.655×PX`，未扣其他加工及物流成本；玻璃生产利润=`FG−0.2×SA`，未扣燃料与其他成本。四项均使用 xtdata `JQ00` 持仓量加权序列，并在月份展开区按同到期月计算；平衡手数分别按 `1:1`、`2:3`、单边不超过 20 手时最接近 0.655 单耗的 `20:13`，以及玻璃利润公式系数对应的 `5:1` 展示。
