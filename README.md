...
- 海外相对指标：纳斯达克/标普500使用两只现货指数收盘价；马盘棕榈油和美盘豆油的比价将 `FCPO（林吉特/吨）` 与 `CBOT美豆油（美分/磅）` 分别换算为人民币/公吨后相除，汇率用外管局中间价并以 BNM 最新值校对；美盘油粕比=`CBOT美豆油（美分/磅）×20÷CBOT美豆粕（美元/短吨）`，先把两腿统一为美元/短吨再计算。
