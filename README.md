...
- 商品组合：蛋白质价差（豆粕减菜粕）、豆棕价差及油脂比价均使用 xtdata `JQ00` 加权序列作为默认展示口径。棕榈油/菜油比价=`P/OI`、棕榈油/豆油比价=`P/Y`、菜油/豆油比价=`OI/Y`，三项月份展开均限制为1、5、9月共同合约。科创50/上证50、创业板/沪深300使用 xtdata 现货指数，只作风格参考，因此不展示平衡手数、名义值和保证金。
...
`xtdata` 凭证只从 `XTQUANT_TOKEN` 环境变量或本机 `E:\data\config.py` 读取，不写入项目。若 xtdata 行情不可用、37 组主表数据或3个展开利润观察不齐、批准的外部数据缺失或过期、交易日不一致或检测到未来数据，脚本保留上次有效网页数据并记录 `xtdata_unavailable`、`external_source_unavailable` 或校验错误，不发布异常结果。
