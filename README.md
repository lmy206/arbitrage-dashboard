...
- 价差与利润代理：主表保留 PTA盘面加工费=`TA−0.655×PX`；MTO盘面利润=`PP−3×MA` 放在“聚丙烯/甲醇比价”的展开观察口径中，玻璃生产利润=`FG−0.2×SA` 放在“玻璃/纯碱比价”的展开观察口径中，焦化利润=`J−1.3×JM` 放在“焦炭/焦煤比价”的展开观察口径中，三项均排在各自加权行上方。全部使用 xtdata `JQ00` 持仓量加权序列；展开利润观察配比分别按 `2:3`、`5:1` 与 `6:13` 展示，并各自提供分位判断与历史图。
