...
3. 只有在 `status=ok`、`pairCount=expectedPairCount=36`、`futureDataDetected=false`、`hierarchySorted=true`、`relatedObservationsComplete=true` 且 `externalSourcesComplete=true` 时，才运行：
